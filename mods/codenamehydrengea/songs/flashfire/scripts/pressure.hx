...
function createPressureHUD() {
    ...
    // Position near HP bar, accounting for downscroll
    if (PlayState.instance.downscroll) {
        pressureText.y = 640;
    } else {
        pressureText.y = FlxG.height - 70;
    }
    pressureText.x = 90; // position on the left side
...
function updatePressureText() {
    ...
    // Color coding based on pressure threshold
    if (percent >= 100) {
        pressureText.color = FlxColor.RED; // at cap
    } else if (percent >= 67) {
        pressureText.color = 0xFFFF8800; // orange
    } else if (percent >= 34) {
        pressureText.color = FlxColor.YELLOW; // middle
    } else {
        pressureText.color = FlxColor.WHITE; // low
    }
}
